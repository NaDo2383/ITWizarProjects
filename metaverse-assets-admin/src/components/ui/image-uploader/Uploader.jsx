...
import SettingServices from '@/common/services/SettingServices';
import { notifyError, notifySuccess } from '@/utils/toast';
import Container from '@/components/ui/image-uploader/Container';
...
function Uploader({ setImageUrl, imageUrl, product, folder }) {
    ...
    useEffect(() => {
        ...
        if (files) {
            files.forEach((file) => {
                if (
                    product &&
                    imageUrl?.length + files?.length > globalSetting?.number_of_image_per_product
                ) {
                    return notifyError(
                        `Maximum ${globalSetting?.number_of_image_per_product} Image Can be Upload!`,
                    );
                ...
                const publicId = name?.substring(0, name.lastIndexOf('.'));
