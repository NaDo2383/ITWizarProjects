...
const useStaffSubmit = (id) => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const nameTranslates = await handlerTextTranslateHandler(data.name, language);
            ...
            const staffData = {
                ...
                phone: data.phone,
                role: data.role,
                joiningDate: selectedDate ?? dayjs(new Date()).format('YYYY-MM-DD'),
                image: imageUrl,
                lang: language,
            };
    ...
    }, [id, setValue, isDrawerOpen, authState.email, clearErrors]);
