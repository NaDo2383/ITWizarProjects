...
const RecentCartImage = ({
		...
		const onLoadImg = ({ target: img }) => {
			// console.log(name,img.naturalHeight)
			// console.log(name, img.naturalWidth)
			// console.log(name, img.offsetHeight);
			// console.log(name, img.offsetWidth);
			if (imageRef) {
		...
		useEffect(() => {
			if (status === "ADMIN_PENDING" || status === "GASFEE_PENDING") {
				setIsHover(false)
			}
		})
		...
		return (
			<>
				<div
					...
					ref={containerRef}
					className={` relative h-full w-full ${status === "ADMIN_PENDING" || status === "GASFEE_PENDING" ? " pointer-events-none" : "cursor-pointer"}`}>
					{isAuction && <Mark auction={auction} />}
					{fileType === "VIDEO" ? (
...
export default RecentCartImage;
