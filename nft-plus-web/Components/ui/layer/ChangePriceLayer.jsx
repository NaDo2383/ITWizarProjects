...
function OrderPaymentLayer(props) {
    const { isOpen, buyDetails, type, currency, close, makePayment } = props;
        ...
        <MainLayer isOpen={isOpen}>
            <div className="px-[25px] pt-[15px] pb-[20px]">
                <div className=" flex  justify-between pb-3 border-b border-[#000000] items-center">
                    <h3 className="text-[22px] font-medium">{orderPaymentI18}</h3>
                ...
                <div className="flex items-center justify-between border-b  py-6 px-[0px]">
                    <div className="flex items-center">
                        <div className="relative w-[140px] h-[117px] overflow-hidden rounded-xl">
                            <Image unoptimized priority src={buyDetails?.thumbnail ? buyDetails?.thumbnail : defart} alt="thumbnail" objectFit="cover" layout="fill" />
                        </div>
                        <div className="ml-5  w-56">
                            <h3 className="font-semibold text-[20px]">{buyDetails.title}</h3>
                            <div className="flex items-center mt-2">
                                <div className="relative w-[27px] h-[27px] rounded-full overflow-hidden bg-[#333] border-[#999] border">
                                    <span
                                        ...
                                        <Image priority unoptimized layout="fill" objectFit="cover" src={buyDetails.owner.avatar ? buyDetails.owner.avatar : defpro} alt="owner-avatar" />
                                    </span>
                                </div>
                                <span className="font-normal ml-2 text-[16px]">{buyDetails.owner.name}</span>
                            </div>
                            <div className=" text-[15px] mt-3 md:hidden font-normal">
                                <span>{Web3.utils.fromWei((buyDetails?.price || "0").toString(), "ether")}</span> {buyDetails.currency}
                            </div>
                        </div>
                    </div>
                    <div className=" text-[18px] text-bold text-[#DDD] hidden md:flex font-bold">
                        ...
                        {Web3.utils.fromWei((buyDetails?.price || "0").toString(), "ether")}{buyDetails.currency}
                    </div>
                </div>
                <div className="flex items-center justify-between px-0 py-3  font-medium text-[20px] h-[53px]">
                    <div>{totalI18}</div>
                    <div>
                        <span className="font-bold">{Web3.utils.fromWei((buyDetails?.price || "0").toString(), "ether")}</span> {buyDetails.currency}
                    </div>
                </div>
...
function OrderPaymentTerms({ updateAcceptTerms, acceptTermsError }) {
    ...
    return (
        <div>
            <h3 className="text-center text-[20px] mt-[16px] font-medium">{orderPurchaseI18}</h3>
            <div
                style={{
