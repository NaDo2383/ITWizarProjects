...
} from "@/common/storage/cookieStorage"
...
export function setRefreshTokenIntoCookie(token) {
    setRefreshTokenCookie(token)
}

export function endToken() {
    window !== undefined && removeCookie(CookieName.TOKEN)
    window !== undefined && removeCookie(CookieName.REFRESH_TOKEN)
    return
}
